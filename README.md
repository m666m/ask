...
bash-completion（bash）或 zsh（自带补全系统）为可选依赖，安装后可启用 Tab 键命令补全的 AI 提示。详见 [命令自动完成](#命令自动完成)。

## 安装

1、运行安装脚本，它会从 GitHub 下载 ask 及其 bash/zsh 命令行自动完成脚本，安装到用户级目录
...
PS. 如有自定义下载函数，先 export -f 导出函数，再通过 ASK_INSTALL_CURL 传入：
    ...
    $ export -f curlgh
    $ curlgh https://github.com/m666m/ask/raw/main/install.sh | ASK_INSTALL_CURL=curlgh bash
...
推荐用法：[把 tmux 面板内容发送给 AI]

### 命令自动完成

ask 可借助 shell 的补全系统自动提示可用的命令用法，适合想不起来命令参数的场景。**bash 和 zsh 均支持。**

> **macOS 用户注意**：bash 补全脚本使用了 `mapfile`（bash 4+ 内建命令），macOS 自带的 bash 3.2 不兼容。请通过 Homebrew 安装新版 bash（`brew install bash`），再补全功能即可正常使用。zsh 用户不受影响。
...
1. 下载 ask 命令脚本和 shell 自动完成脚本到用户级目录，并设置权限

    $ mkdir -p ~/.local/bin ~/.local/share/bash-completion/completions ~/.local/share/zsh/site-functions
    ...
    $ # zsh 补全
    $ curl -fsSL https://github.com/m666m/ask/raw/main/_ask -o ~/.local/share/zsh/site-functions/_ask
